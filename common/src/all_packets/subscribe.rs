...
impl Topic {
    fn read_from(stream: &mut dyn Read) -> Result<Topic, std::io::Error> {
        ...
        let mut qos_level_bytes = [0u8; 2];
        stream.read_exact(&mut qos_level_bytes)?;
        let qos_level = u16::from_be_bytes(qos_level_bytes);
...
impl Subscribe {
    //NO SE DEBERIA PERMITIR QUE NO TENGA AL MENOS UN TOPIC -> TOPIC INICIAL EN NEW? RECIBIR VECTOR?
    ...
    fn get_remaining_length(&self) -> Result<u32, String> {  
        ...
        for topic in &self.topics {
            length += encode_mqtt_string(&topic.name)?.len() as u8 + /*topic.qos as u8*/1;
        }
...
impl WritePacket for Subscribe {
    fn write_to(&self, stream: &mut dyn Write) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let first_byte = SUBSCRIBE_PACKET_TYPE;
...
fn verify_subscribe_byte(byte: &u8) -> Result<(), String>{
    match *byte {
        SUBSCRIBE_PACKET_TYPE => return Ok(()),
...
mod tests {
    ...
    fn correct_remaining_length() {
        ...
        subscribe.add_topic(Topic{name: String::from("test"), qos: Qos::AtMostOnce});
    ...
    fn correct_subscribe_packet() {
        ...
        subscribe_packet.add_topic(Topic{name: String::from("otro test"), qos: Qos::AtLeastOnce});
        let mut buff = Cursor::new(Vec::new());
        subscribe_packet.write_to(&mut buff).unwrap();
        buff.set_position(1);
        let to_test = Subscribe::read_from(&mut buff, 0x80).unwrap();
